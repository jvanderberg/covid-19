...
file_date = datetime.datetime(data['lastUpdatedDate']['year'],
                              data['lastUpdatedDate']['month'], data['lastUpdatedDate']['day'])
    ...
    for age_group in age_groups:
        try:
            ...
            group = {"age_group": age_group,
                     "count": group['count'], "tested":  group['tested']}
        except:
    ...
    if (bad_file == False or last_good_file):
        ...
        try:
            fileDate = datetime.datetime(
                data['LastUpdateDate']['year'], data['LastUpdateDate']['month'], data['LastUpdateDate']['day'])
        except:
            fileDate = datetime.datetime(
            ...
            if (bad_file):
                ...
                if (demographics_exist):
                    ...
                    for age_group in zip_data['demographics']['age']:
                        table[age_groups[age_index] +
                              " count"].append(age_group['count'])
                        table[age_groups[age_index] +
                              " tested"].append(age_group['tested'])
                        table[age_groups[age_index] + " percentage"] = 0
                        table[age_groups[age_index] +
                              " count_14day"].append(np.NaN)
                        table[age_groups[age_index] +
                              " tested_14day"].append(np.NaN)
...
for age_group in age_groups:
    diff[age_group+' percentage'] = diff[age_group +
                                         ' count'] / diff[age_group+' tested']
...
for age_group in age_groups:
    ...
    diff[age_group+' percentage_14day'] = diff[age_group +
                                               ' count_14day'] / diff[age_group+' tested_14day']
...
for age_group in age_groups:
    dftotals[age_group+' percentage'] = dftotals[age_group +
                                                 ' count'] / dftotals[age_group+' tested']
...
for age_group in age_groups:
    dftotals[age_group+' percentage'] = dftotals[age_group +
                                                 ' count'] / dftotals[age_group+' tested']
    dftotals[age_group+' count_14day'] = dftotals[age_group +
                                                  ' count'].rolling(window=14).mean()
    dftotals[age_group+' tested_14day'] = dftotals[age_group +
                                                   ' tested'].rolling(window=14).mean()
    dftotals[age_group+' percentage_14day'] = dftotals[age_group +
                                                       ' count_14day'] / dftotals[age_group+' tested_14day']
