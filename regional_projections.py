...
df = pd.read_csv('regional_all.csv', header=[0,1], index_col=0, parse_dates=True)

cfr = pd.DataFrame({'deaths':df['deaths']['Illinois'],'cases': df['count']['Illinois'], 'tests': df['tested']['Illinois']})

cfr = cfr[cfr.index > startdate]
cfr = cfr.rolling(window=14, center=True).mean()
...
max_corr_lag = 18
future_index = pd.date_range(cfr.tail(1).index[0]+datetime.timedelta(days=1), periods=max_corr_lag)
...
plt.plot(cfr['percentage_lagged'], cfr['fit'], label='Model Slope {:0.1f} deaths/% R² {:0.3f}'.format(slope/100,r_value**2))
